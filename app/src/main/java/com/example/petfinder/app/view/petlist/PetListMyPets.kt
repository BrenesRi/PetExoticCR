...
import com.example.petfinder.app.view.mypets.MyPetsDirections
import com.example.petfinder.app.viewmodel.PetViewModel
import com.example.petfinder.app.viewmodel.ViewModelFactory
import com.example.petfinder.databinding.FragmentPetListMyPetsBinding

class PetListMyPets : Fragment() {
    private lateinit var binding: FragmentPetListMyPetsBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentPetListMyPetsBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.rvPetListMyPets.layoutManager = LinearLayoutManager(requireContext())
        binding.rvPetListMyPets.adapter = adapter
        ...
        petViewModel.petList.observe(viewLifecycleOwner) { pets ->
            adapter.setPetList(pets)
        }
        petViewModel.findMyPet()
    }

    private fun onPetClicked(petItem: PetItem) {
        when (petItem) {
            is PetItem.PetType -> {
                val action = MyPetsDirections.actionMyPetsScreenToEditPet(petItem.pet.id)
                findNavController().navigate(action)
            }
            is PetItem.AdoptionRequestType -> {
                // Handle this case if needed, or ignore it if this screen only deals with pets
            }
        }
    }
}